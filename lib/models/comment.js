...
var Vote = new Schema({
  author: { type: ObjectId, ref: 'User', required: true },   value: { type: String, enum: [ 'positive', 'negative' ], required: true },   createdAt: { type: Date, default: Date.now }
})
...
var Flag = new Schema({
  author: { type: ObjectId, ref: 'User', required: true },   value: { type: String, enum: [ 'spam' ], required: true },   createdAt: { type: Date, default: Date.now }
})
...
var CommentReplySchema = new Schema({
  author: {type: ObjectId, required: true, ref: 'User'},   text: {type: String, validate: replyValidator, required: true },   createdAt: { type: Date, default: Date.now },   editedAt: { type: Date }
})
...
var CommentSchema = new Schema({
  author: { type: ObjectId, required: true, ref: 'User' },   text: { type: String, validate: commentValidator, trim: true, required: true, set: reduceLB },   replies: [ CommentReplySchema ], // Reference to the ObjectId of the Discussion Context   reference: { type: Schema.Types.Mixed, required: true }, // Discussion Context   context: { type: String, required: true, enum: ['proposal', 'topic', 'clause', 'body', 'paragraph'] }, // If the context is clause or body, we save a reference to the topic to get the Side Comments of a Topic // in a straightforward way   topicId: { type: ObjectId },   votes: [ Vote ],   score: { type: Number, default: 0 },   flags: [ Flag ],   createdAt: { type: Date, default: Date.now },   editedAt: { type: Date }
})
