...
var mongoUri = config('mongoUsersUrl');

uriObj = muri(mongoUri);

 var dbopts = {
   cluster: uriObj.hosts.map(serialize).join(','),
   dbname: uriObj.db,
   replSetName: uriObj.options.replicaSet,
   mongoUser: uriObj.auth.user,
   mongoPassword: uriObj.auth.pass
 };

// db.connect(config('mongoUrl'), dbopts);

var conn = db.connect('users', dbopts).connection;
