...
TopicSchema.methods.poll = function (user, value, cb) {
  if (this.status === 'recount') {
    return cb(new Error('Polling is closed on recount.'))
  }
...
/**
 * Undo poll Topic from provided user
 *
 * @param {User|ObjectId|String} user
 * @param {Function} cb
 * @api public
...
TopicSchema.methods.unpoll = function (user, cb) {
  const polls = this.action.pollResults
  const c = user.get ? user.get('_id') : user

  const polled = polls.filter(function (v) {
    var a = v.author.get ? v.author.get('_id') : v.author
    return a.equals ? a.equals(c) : a === c
  })

  log('About to remove polls %j', polled)

  if (polled.length) {
    polled.forEach((v) => {
      const removed = polled.id(v.id).remove()
      log('Remove vote %j', v.id)
    })
  }
