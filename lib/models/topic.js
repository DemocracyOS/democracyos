...
TopicSchema.methods.support = function (user, cb) {
  if (this.status === 'closed') {
    return cb(new Error('Supporting is closed.'))
  }

  // Here we could provide a 5000ms tolerance (5s)
  // or something... to prevent false positives
  if (this.closingAt && (+new Date(this.closingAt) < +new Date())) {
    return cb(new Error('Can\'t support after closing date.'))
  }

  if (~this.action.causeResults.map((r) => r.author.toString()).indexOf(user._id.toString())) {
    return cb(new Error('Can\'t support twice.'))
  }

  const poll = { author: user }

  this.action.causeResults.push(poll)
