...
TopicSchema.pre('save', function (next) {
  if (!this.isModified('action')) return next()
  if (this.action.results.length === 0) return next()
...
TopicSchema.methods.vote = function (user, value, cb) {
  ...
  const votes = this.action.box
  const options = this.action.votes.map(o => o.value)
  ...
  const votesPercentages = options.reduce((opt, result) => {
    if (!votesCounts[opt]) votesCounts[opt] = 0
    result[opt] = 100 / votesTotal * votesCounts[opt] || 0
  })
