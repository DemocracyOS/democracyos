...
var Vote = new Schema({
  author: { type: ObjectId, ref: 'User', required: true },   value: { type: String, enum: ['positive', 'negative', 'neutral'], required: true },   trustee: { type: ObjectId, ref: 'User' },   caster: { type: ObjectId, ref: 'User' },   createdAt: { type: Date, default: Date.now },   democracy: { type: ObjectId, ref: 'Deployment' }
})
...
var ClauseSchema = new Schema({
  clauseName: { type: String },   order: { type: Number, required: true, default: 0 },   text: { type: String, required: true, default: 'Undefined text' },   centered: { type: Boolean, required: true, default: false }
})
...
var LinkSchema = new Schema({
  text: { type: String },   url: { type: String, validate: validators.isURL({ skipEmpty: true }) }
})
...
var ParagraphSchema = new Schema({
  markup: { type: String },   position: { type: Number },   empty: { type: Boolean, default: false }
})
...
var TopicSchema = new Schema({
  topicId: { type: String },   tag: { type: ObjectId, ref: 'Tag', required: true },   officialTitle: { type: String, required: false },   mediaTitle: { type: String, required: false },   author: { type: String },   authorUrl: { type: String, validate: validators.isURL({ skipEmpty: true }) },   source: { type: String, validate: validators.isURL({ skipEmpty: true }) },   clauses: [ParagraphSchema],   votes: [Vote],   participants: [{type: ObjectId, ref: 'User' }],   createdAt: { type: Date, default: Date.now },   updatedAt: { type: Date },   closingAt: { type: Date },   publishedAt: { type: Date },   deletedAt: { type: Date },   votable: { type: Boolean, required: true, default: true },   bodyTruncationText: { type: String },   links: [LinkSchema],   forum: { type: ObjectId, ref: 'Forum', required: false }
})
