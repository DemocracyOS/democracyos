...
var TopicSchema = new Schema({
  ...
  // , body: { type: [ParagraphSchema], required: false }
  , clauses: [String]
  , votes: [Vote]
...
TopicSchema.methods.recount = function(cb) {
  if (+new Date(this.closingAt) < +new Date) {
    log('Deny to recount topic before closing date.');
    return cb && cb(new Error('Deny to recount topic before closing date.'));
  }
  this.status = 'recount';
  cb && this.save(cb);
}
