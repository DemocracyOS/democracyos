...
var log = new Log('democracyos:forum-model')
...
ForumSchema.post('save', adoptOrphansTopicsOnSingleForum)
...
function adoptOrphansTopicsOnSingleForum (forum) {
  if (!config.multiForum) {
    var api = require('lib/db-api')
    api.topic.all({forum: ''}, function (err, topics) {
      if (err) {
        log('Error fetching orphan topics: %s', err)
      }
      if (topics.length > 0) {
