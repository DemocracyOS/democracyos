...
function postTopics (req, res, next) {
  switch (req.keysToUpdate['action.method']) {
    case 'vote':
      req.keysToUpdate['action.results'] = [{ value: 'positive', percentage: 0 }, { value: 'neutral', percentage: 0 }, { value: 'negative', percentage: 0 }]
      break
    case 'poll':
      req.keysToUpdate['action.results'] = req.body['action.options'].map((o) => ({ value: o, percentage: 0 }))
      break
    case 'cause':
      req.keysToUpdate['action.results'] = [{ value: 'support', percentage: 0 }]
      break
    default:
      req.keysToUpdate['action.results'] = []
  }
