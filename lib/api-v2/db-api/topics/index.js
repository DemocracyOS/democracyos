...
function doVote (user, value, topic) {
  ...
  return new Promise ((resolve, reject) => {
    votedBy(user, topic).then(voted => {
      ...
      newVote.save().then(() => {
        Vote.find({topic: topic.id}).then(votes => {
          topic.action.count = votes.length
          ...
          options.forEach((opt) => {
            ...
            votesPercentages[opt] = 100 / topic.action.count * votesCounts[opt] || 0
          })

          topic.action.results = options.map((opt) => ({
            value: opt,
