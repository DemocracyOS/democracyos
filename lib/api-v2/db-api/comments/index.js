...
exports.removeComment = function remove (opts) {
  ...
  return find()
    ...
    .select(scopes.ordinary.select)
    .exec()
    .then(verifyCommentAuthor.bind(null, user))
    .then(verifyNoReplies.bind(null, user))
    .then(doRemoveComment)
}

function verifyCommentAuthor (user, comment) {
  if (!comment.author.equals(user._id)) {
    const err = new Error('Can delete comments from other users')
    err.code = 'NOT_YOURS'
    throw err
  }
  return comment
...
function verifyNoReplies (user, comment) {
  if (comment.replies.length > 0 && !user.staff) {
    const err = new Error('Can delete comments with replies')
...
exports.removeReply = function remove (opts) {
  ...
  return find()
    ...
    .select(scopes.ordinary.select)
    .exec()
    .then(verifyReplyAuthor.bind(null, user, replyId))
    .then(doRemoveReply.bind(null, user, replyId))
    .then((comment) => scopes.ordinary.expose(comment, user))
}

function verifyReplyAuthor (user, replyId, comment) {
...
function doFlag (user, value, comment) {
  return new Promise((resolve, reject) => {
