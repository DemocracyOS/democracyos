...
const log = require('debug')('democracyos:notifications-v2')
...
exports.commentReply = function commentReply (req, res, next) {
  ...
  User.get(req.comment.author.id, function(err, commentAuthor) {
    ...
    const topicUrl = config.protocol + '://' + config.host + urlBuilder.for('site.topic', {forum: req.forum.name, id: req.topic.id})
