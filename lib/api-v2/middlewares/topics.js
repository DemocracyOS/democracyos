...
exports.parseUpdateableKeys = function parseUpdateableKeys (req, res, next) {
  ...
  if (req.topic.action.count > 0 && req.topic.action.method !== req.body['action.method']) {
    const err = new Error("Can't change method after votes casted")
    next(err)
  }

  if (req.topic.action.count > 0 && !isEqual(req.topic.action.options, req.body['action.options'])) {
