...
app.get('/:forum/comments/csv',
middlewares.users.restrict,
middlewares.forums.privileges.canChangeTopics,
function getCommentsCsv (req, res, next) {
  const forumId = config.multiForum ? req.query.forum : config.defaultForum
  api.comments.getCsv({forumId})
  .then((commentCsv) => {
    res.status(200)
    res.set({
