...
exports.clearAll = function() {
  debug('clear fields, notes, messages');
...
exports.enable = function(selector) {
  o('button', selector).attr('disabled', false);
};
...
exports.field = function(name, options, val, print, fn) {
  debug('validate `%s` - %s', name, val, options);
  ...
  for (var key in options) {
    ...
    cb(val, options[key], function(err) {
      ...
      --pending || function() {
        debug('validation for `%s`: %s', name, ok);
        return fn(ok);
      }();
    });
  }
