...
function build(done, locale) {
  ...
  function onresolve(err, tree) {
    ...
    var batch = new Batch();
    batch.push(buildWrapper(locale));
    ...
    function buildWrapper(locale) {
      return function buildScripts(done) {
        builder.scripts(tree)
          .use('scripts', builder.plugins.js())
          .use('json', translations({ locale: locale }))
...
function middleware (req, res, next) {
  ...
  function onbuild(locale) {
    return function (err, res) {
      if (err) return next(err);

      log('Write script and stylesheet for locale ' + locale);
      write('public/app.' + locale + '.js', res.js);
      write('public/app.css', res.css);
    }
  }

  var locales = ['ca', 'de', 'en', 'es', 'fr', 'fi', 'gl', 'it', 'nl', 'pt', 'ru', 'sv','uk'];
  locales.forEach(function(locale) {
    build(onbuild(locale), locale);
  });
