...
CommentsView.prototype.newArgument = function(ev) {
  ...
  var el = o('.comment-form.upper', this.el);
...
CommentsView.prototype.loadMyArguments = function(comments) {
  if (comments.length) {
    ...
    var el = o('.comment-form.lower', this.el);
    classes(el).add('hide');
  }
...
CommentsView.prototype.addMyArgument = function(comment) {
  var form = o('.comment-form.upper', this.el);
  classes(form).add('hide');
  classes(o('.comment-form.lower', this.el)).remove('hide');
      ...
      if (err) {
        ...
        setTimeout(function () {
          ...
          if (!self.myArguments.length) {
            classes(o('.comment-form.upper', self.el)).remove('hide');
            classes(o('.comment-form.lower', self.el)).add('hide');
          }
        }, 2500)
      }
  });
