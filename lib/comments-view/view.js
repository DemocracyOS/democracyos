...
CommentsView.prototype.validate = function(data) {
  var errors = [];
  if (!data.text) {
    errors.push({msg: 'Comment cannot be empty'});
  };
  if (data.text.length > 2048) {
    errors.push({msg: 'Comment is limited to 2048 characters'});
  };
  return errors;
}
...
CommentsView.prototype.errors = function(errors) {
  var div = this.el.querySelector('div.form-errors');
  ...
  div.innerText = '';
  for (error in errors) {
    div.innerText += t(errors[error].msg);
  }
...
CommentsView.prototype.post = function(data) {
  ...
  .end(function(err, res) {
    ...
    if (res.body && res.body.errors.length) {
      view.errors(res.body.errors);
      return;
    }
    if (!res.ok) {
      log('Fetch error: %s', res.error);
