...
CommentsView.prototype.add = function(comment) {
  ...
  if (!comment.deleted) {
    ...
    tip(flag);
    var btnRemove = o('.btn-remove', commentEl);
    var btnEdit = o('.btn-edit', commentEl);
    if (btnRemove) tip(btnRemove);
    if (btnEdit) tip(btnEdit);
...
CommentsView.prototype.onedit = function(ev) {
  ...
  var target = ev.delegateTarget || closest(ev.target, 'a');
  var comment = closest(target, '.media-body');
  classes(comment).add('edit');
};
...
CommentsView.prototype.onsuccessedit = function(data) {
  var commentText = o('li[data-id=' + data.id + '] .comment-text', this.el);
  var commentTime = o('li[data-id=' + data.id + '] .comment-time', this.el);
