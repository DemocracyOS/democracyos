...
CommentsView.prototype.switchOn = function() {
  ...
  this.events.bind('click a.replies-counter', 'onshowreplies');
...
CommentsView.prototype.onshowreplies = function(ev) {
  ev.preventDefault();
  ...
  request
  .get('/api/comment/:id/replies'.replace(':id', id))
  .end(function(err, res) {
    if (err) {
...
CommentsView.prototype.add = function(comment) {
  ...
  if (!comment.deleted) {
    ...
    commentsRepliesView.render(o('.comment-actions', commentEl));
    commentsRepliesView.on('click', this.renderReplies.bind(this));
  } else {
...
/**
 * Render replies in container
 */

CommentsView.prototype.renderReplies = function (commentsRepliesView) {
  var el = commentsRepliesView.el;
  var commentEl = closest(el, 'li[data-id]');

  commentsRepliesView.renderReplies(o('.comment-replies', commentEl));
};
