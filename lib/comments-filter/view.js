...
export default class CommentsFilter extends View {
  constructor () {
    super(template);
    this.refresh();
    this.locals = { label: this.get().label, sorts: sorts };
    this.build();
  }
  ...
  refresh (sort) {
    if (!citizen.logged()) {
      ...
      store.get('comments-filter', (err, data) => {
        if (err) log('unable to fetch');
        this.set(data ? data : sorts.score);
      });
    }
  }
  ...
  set (sort) {
    ...
    if (citizen.logged()) {
      store.set('comments-filter', this.get(), ((err, ok) => err ? log('unable to save') : log('saved')));
    }
