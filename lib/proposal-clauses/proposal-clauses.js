...
export default class ProposalClauses extends View {
  ...
  load () {
    ...
    request
      ...
      .end((err, res) => {
        ...
        request
          .get('/api/topic/:id/body-comments'.replace(':id', self.topic.id))
          .end((err, res) => {
            if (res.status !== 200) ; //log error
  ...
  getSections (topic) {
    ...
    let sections = [];
    let body = o(topic.body);
    let paragraphs = body[0].childNodes;
  ...
  getClausesSections (topic) {
    ...
    let self = this;
    let paragraphs = topic.body.split('\n');
    let sections = [];
