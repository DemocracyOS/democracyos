...
function topicsByForum(ctx, next) {
  topicStore.setForum(ctx.forum.name)
  topicStore
    .all()
    .then(topics => {
      ctx.topics = topics;
...
page(forumRouter('/admin/topics'), forumStore.getFromParamsMiddleware, topicsByForum, (ctx, next) => {
...
page(forumRouter('/admin/topics/create'), tags.middleware, (ctx, next) => {
  sidebar.set('topics');
...
page(forumRouter('/admin/topics/:id'), tags.middleware, loadTopic, (ctx, next) => {
...
page(forumRouter('/admin/tags'), tags.middleware, (ctx, next) => {
...
page(forumRouter('/admin/tags/:id'), loadTag, (ctx, next) => {
  // force section for edit
  // as part of list
  sidebar.set('tags');
