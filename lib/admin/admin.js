...
var LawsList = require('admin-laws').List;
...
page('/admin/laws/:id', tags.middleware, load, function (ctx, next) {
  var content = o('.admin-content');
...
function valid(ctx, next) {
  ...
  if (/laws/.test(section)) return next();
  if (/laws\/create/.test(section)) return next();
  if (/laws\/[a-z0-9]{24}\/?$/.test(section)) return next();
}
