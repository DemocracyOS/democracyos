...
page(urlBuilder.for('admin.topics.create'), privileges('canChangeTopics'), clearTagStore, findAllTags, (ctx) => {
  ctx.sidebar.set('topics')
...
page(urlBuilder.for('admin.topics.id'), privileges('canChangeTopics'), clearTagStore, findAllTags, findTopic, (ctx) => {
...
page(urlBuilder.for('admin.comments'), (ctx) => {
  const exportComments = new ExportComments({forum: ctx.forum.name})
