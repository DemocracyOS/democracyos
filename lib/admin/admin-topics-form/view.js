...
export default class TopicForm extends FormView {
  constructor (topic, forum, tags) {
    ...
    if (topic) {
      ...
      locals.form.action = '/api/topic/create'
  ...
  onsuccess (res) {
    ...
    if (this.topic) {
      topicStore.unset(this.topic.id).parse(res.body.results.topic)
        .then((topic) => { topicStore.set(topic.id, topic) })
        .catch((err) => { throw err })
    }
  ...
  addLink () {
    ...
    request
      .post(`/api/topic/${this.topic.id}/link`)
  ...
  onremovelinkclick (ev) {
    ...
    var link = closest(ev.target, '[data-link]')
