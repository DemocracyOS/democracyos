...
export default class TopicForm extends FormView {
  ...
  switchOn () {
    ...
    this.bind('click', 'a.auto-save', this.bound('onsaveclick'))
  ...
  onsuccess (res) {
    ...
    topicStore.parse(res.body.results.topic)
      .then((topic) => {
        ...
        if (this.forum.visibility === 'collaborative' && !topic.privileges.canChangeTopics) {
          return page(urlBuilder.for('site.topic', { forum: this.forum.name, id: topic.id }))
        }
