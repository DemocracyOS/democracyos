...
import CustomAttrs from './custom-attrs/component'
...
export default class TopicForm extends FormView {
  ...
  switchOn () {
    ...
    Array.prototype.forEach.call(this.el[0].querySelectorAll('input[type="tags"]'), tagsInput)

    ReactRender(<ForumTagsSearch tags={this.topic && this.topic.tags && this.topic.tags} forum={this.forum.id} />, this.el[0].querySelector('.tags-autocomplete'))

    if (this.forum.topicsCustomAttrs.length > 0) {
      const customAttrs = this.el[0].querySelector('[data-custom-attributes]')

      ReactRender(
        <CustomAttrs forum={this.forum} topic={this.topic} />,
        customAttrs
      )
    }
  }
