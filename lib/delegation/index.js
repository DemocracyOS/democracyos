...
var loadDelegation = function(req, res, next) {
  ...
  api.delegation.getByTrusterAndTag(req.user.id, req.scope, function(err, delegation) {
...
var createDelegation = function(req, res, next) {
  ...
  if (req.user.id == trusteeId) {
    log('Impossible to create a self-delegation for user %s', req.user.id);
    return next();
  } 
