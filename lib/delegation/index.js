...
app.post('/delegations/create', restrict, required('tag tid'), loadDelegation, createOrUpdateDelegation, function(req, res) {
  res.json(req.delegation);
});

app.post('/delegations/remove', restrict, required('tag'), loadDelegation, removeDelegation, function(req, res) {
    res.send(200, 'OK');
});
...
function _handleError (err, req, res) {
  res.format({
    html: function() {
      ...
      // maybe with flash or even an error page.
...
var createOrUpdateDelegation = function(req, res, next) {
  var tag = req.param('tag');
...
var removeDelegation = function(req, res, next) {
