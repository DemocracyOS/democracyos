...
let log = debug('democracyos:democracy-model');

export default class Deployment extends Emitter {
  ...
  fetch () {
    ...
    this.$_req = request
      ...
      .end((err, res) => {
        ...
        let democracy = res.body;
        if (!democracy) return this.status('non-existent');
        if ('object' !== typeof democracy) throw new Error('Bad response from server.');
        this.attrs(democracy);
      });
