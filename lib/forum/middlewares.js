...
module.exports.forum = function (req, res, next) {
  ...
  if (req.body.name) {
    Forum.findOneByName(req.body.name, found);
...
module.exports.private = function (req, res, next) {
  var forum = req.forum;
  var user = req.user;
  if (user && forum && (!forum.private || forum.isAdmin(user) || forum.hasRole('admins', user || forum.hasRole('collaborators', user)))) {
    return next();
  } else {
    log('forum not found');
    return res.status(404).send();
