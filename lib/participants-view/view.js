...
View.prototype.vote = function (id) {
  ...
  if (!alreadyVoted) {
    this.participants = this.participants.concat(citizen.id);
    var boxEl = o('.participants-box');
    this.el = render.dom(box, { participants: this.participants});
    boxEl.parentNode.replaceChild(this.el, boxEl);
    this.fetch();
  }
}
