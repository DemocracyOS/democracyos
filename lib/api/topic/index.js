...
var forumPrivilegesMiddleware = forumMiddlewares.privileges
...
app.get('/topic/all',
  ...
  forumPrivilegesMiddleware('canView'),
  function (req, res, next) {
    if (req.query.draft) {
      forumPrivilegesMiddleware('canChangeTopics')(req, res, next)
    } else {
      next()
    }
  },
...
app.get('/topic/:id', forumPrivilegesMiddleware('canView'), function (req, res) {
  ...
  migrateTopic(req.topic, function (_err, topicMigrated) {
    ...
    var topic = exposeTopic(topicMigrated, req.user)
    if (req.user) {
      topic.privileges = privileges.all(req.forum, req.user, topicMigrated)
    } else {
      topic.privileges = {}
    }
    res.json(topic)
...
app.get('/topic/:id/sidecomments', forumPrivilegesMiddleware('canView'), function (req, res) {
...
app.get('/topic/:id/comments', forumPrivilegesMiddleware('canView'), function (req, res) {
...
app.get('/topic/:id/my-comments', restrict, forumPrivilegesMiddleware('canView'), function (req, res) {
...
app.post('/topic/:id/comment',
  restrict,
  forumPrivilegesMiddleware('canVoteAndComment'),
  function (req, res) {
...
app.post('/topic/create',
  restrict,
  forumMiddlewares.findForumById,
  forumPrivilegesMiddleware('canChangeTopics'),
  function (req, res, next) {
...
app.post('/topic/:id/publish',
  ...
  forumPrivilegesMiddleware('canChangeTopics'),
  topicMiddlewares.findTopic,
  topicMiddlewares.privileges('canPublish'),
  function (req, res) {
