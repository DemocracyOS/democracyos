...
function sendValidationEmail(citizen, event, meta, callback) {
  api.token.createEmailValidationToken(citizen, meta, function (err, token) {
    ...
    log('CHECKING NOTIFIER CONFIG');
    log('PROTOCOL: ' + notifier.options.protocol);
    log('HOST: ' + notifier.options.host);
    log('PORT: ' + notifier.options.port);
    log('PATH: ' + notifier.options.path);
    log('TOKEN: ' + notifier.options.token);
    log('ENABLED?: ' + notifier.enabled());

    if (notifier.enabled()) {

      log('SENDING SIGNUP VIA NOTIFIER');
