 ...
 * @return {Module}           [`comment` module]
 */
exports.comments = function getAll(topicId, paging, fn) {
  log('Looking for all comments from %s', topicId);
  query = { reference: topicId, context: { $ne: 'paragraph' }};
  paging = paging || {};
...
/**
 * Get all comments by a user for a particular topic
 * 
 * @param  {ObjectId} topicId [objectId of either topic or platform]
 ...
 * @return {Module}          [`comment` module]
 */
exports.userComments = function userComments(topicId, userId, fn) {
  log('Looking for all comments from %s by %s', topicId, userId);

  query = {reference: topicId, author: userId, context: { $ne: 'paragraph' } };
...
exports.getSideComments = function getSideComments (topicId, paging, fn) {
  log('Querying side comments for %s', topicId);
  paging = paging || {};
