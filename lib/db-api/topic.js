...
exports.all = function all (params, fn) {
  ...
  Topic
    .find(query)
    .select('id topicId mediaTitle tag participants votes createdAt updatedAt closingAt publishedAt deletedAt status open closed links author authorUrl forum coverUrl extra action')
...
function createTopic (data, fn) {
  data.action = {
    method: data.actionMethod
  }
  delete data.actionMethod

  if (data.action.method === 'poll') {
    data.action.pollOptions = data.pollOptions.split(',')
...
exports.update = function update (id, data, fn) {
  ...
  function onget (err, topic) {
    ...
    if (data.action.method === 'poll') {
      ...
      data.action.pollOptions = topic.action.pollOptions
    }

    // save old action data
    data.action.voteResults = topic.action.voteResults
    data.action.pollResults = topic.action.pollResults

    // ensure action changes restriction on initialized voting
    if (topic.participants.length > 0) {
      data.action.method = topic.action.method
      data.action.pollOptions = topic.action.pollOptions
    }
...
exports.comments = function comments (id, paging, fn) {
  ...
  var query = {
    $or: [
      ...
      { context: 'topic', reference: ObjectId(id) }
    ]
  }
