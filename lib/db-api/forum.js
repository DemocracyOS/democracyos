    ...
    if (options['privileges.canChangeTopics']) {
      query.find({
        $or: [
          ...
          {'permissions.user': options['privileges.canChangeTopics']}
        ]
      });
    }
  }
...
exports.findDefaultForum = function findDefaultForum (fn) {
  if (defaultForumCache) return fn(null, defaultForumCache);

  var query = {deletedAt: null};

  if (config.defaultForum) query.name = config.defaultForum;
  ...
  Forum
    ...
    .exec(function (err, forum) {
      ...
      if (!forum) return fn(null);
      defaultForumCache = forum;
      return fn(null, defaultForumCache);
    });
};
