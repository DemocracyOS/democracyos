...
page('/signup', externalSignup, citizen.loggedoff, authFacebookForm, parse, (ctx, next) => {
...
page('/signup/validated', externalSignup, citizen.required, parse, (ctx, next) => {
  // Build form view with options
  ...
  if (ctx.query.reference) {
    setTimeout((() => page('/law/' + ctx.query.reference)), 5000);
  }
});
