...
module.exports = {
  ...
  client: env.CLIENT_CONF ? env.CLIENT_CONF.split(',') : [ "protocol", "host", "publicPort", "env", "locale", "logo", "favicon", "organization name", "organization url", "learn more url", "google analytics tracking id", "comments per page", "spam limit", "faq enabled", "pp enabled", "tos enabled", "glossary enabled" ],
  auth: {
    basic: {
  ...
  "faq": env.FAQ ? ~on.indexOf(env.FAQ.toLowerCase()) : false,
  "tos": env.TERMS_OF_SERVICE ? ~on.indexOf(env.TERMS_OF_SERVICE.toLowerCase()) : false,
  "pp": env.PRIVACY_POLICY ? ~on.indexOf(env.PRIVACY_POLICY.toLowerCase()) : false,
  "glossary": env.GLOSSARY ? ~on.indexOf(env.GLOSSARY.toLowerCase()) : false
};