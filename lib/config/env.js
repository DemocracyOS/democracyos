...
var on = ['on', 'true', '1'];
var fallback = env.NODE_ENV == 'production' ? null : false;
...
module.exports = {
  ...
  faq: env.FAQ ? !!~on.indexOf(env.FAQ.toLowerCase()) || !!env.FAQ : fallback,
  tos: env.TERMS_OF_SERVICE ? !!~on.indexOf(env.TERMS_OF_SERVICE.toLowerCase()) || !!env.TERMS_OF_SERVICE : fallback,
  pp: env.PRIVACY_POLICY ? !!~on.indexOf(env.PRIVACY_POLICY.toLowerCase()) || !!env.PRIVACY_POLICY : fallback,
  glossary: env.GLOSSARY ? !!~on.indexOf(env.GLOSSARY.toLowerCase()) || !!env.GLOSSARY : fallback
};