...
var topicKeys = [
  'id topicId title mediaTitle summary clauses source open closed',
  'tag participants public draft deleted',
...
function(req, res, next) {
  if (req.query.draft) return onlyForumAdmins(req, res, next);
  return next();
},
function (req, res) {
  ...
  api.topic.all({
    ...
    res.json(topics.map(expose(keys)));
  });
});
...
app.post('/topic/create', restrict, hasAccess, function (req, res, next) {
  ...
  api.topic.create(req.body, function (err, topicDoc) {
    ...
    var keys = [
      'id topicId title mediaTitle summary clauses source',
      'open closed public draft deleted tag participants',
      'upvotes downvotes abstentions createdAt updatedAt closingAt',
      'publishedAt deletedAt votable clauseTruncationText links author authorUrl'
    ].join(' ');
