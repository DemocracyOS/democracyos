...
app.get('/topic/:id/body-comments', function (req, res) {
  log('Request /topic/%s/body-comments', req.params.id);
  ...
  api.topic.get(req.params.id, function (err) {
    ...
    api.comment.getFor({context: 'body', reference: re}, paging, function (err, comments) {
      if (err) return _handleError(err, req, res);
