...
app.get('/topic/all', function (req, res) {
  ...
  api.topic.all(function(err, topicDocs) {
  ...
  api.topic.comment(comment, function (err, commentDoc) {
    ...
    if (notifier.enabled()) {
      ...
      notifier.notify(eventName)
        .withData( { topic: req.params.id, user: req.user.id, instance: instanceUrl, comment: commentDoc.text } )
  ...
  .vote(
    ...
    function (err, topic) {
      ...
      if (notifier.enabled()) {
        ...
        notifier.notify(eventName)
          .withData( { topic: topic.id, user: req.user.id, instance: instanceUrl, vote: req.param('value') } )
          .send(function (err, data) {
            if (err) {
              log('Error when sending notification for event %s', eventName);
            } else {
