...
var topicKeys = [
  ...
  'status state tag participants forum',
...
function (req, res) {
  ...
  api.topic.all({
    ...
    var keys = [
      'id topicId mediaTitle status open closed public draft deleted forum',
...
app.post('/topic/:id/vote', restrict, function (req, res) {
  ...
  .vote(
    ...
    function (err, topic) {
      ...
      res.json(expose(topicKeys)(topic.toJSON()));
    }
  );
});
...
app.post('/topic/create', restrict, hasAccess, function (req, res, next) {
  ...
  api.topic.create(req.body, function (err, topicDoc) {
    ...
    var keys = [
      ...
      'status open closed public draft deleted tag participants forum',
      'upvotes downvotes abstentions createdAt updatedAt closingAt',
      'publishedAt deletedAt votable clauseTruncationText links author authorUrl'
    ].join(' ');
