...
exports.facebook = function facebook (profile, callback) {
  ...
  // here we should check email. Send email vexrification and stuff
 ...
 * Forgot password
 *
 * @param {Object} profile object with forgot password data
 * @param {Function} callback Callback accepting `err` and `citizen`
 * @api public
 */

exports.forgotpassword = function forgotpassword (profile, callback) {
  log('password reset requested. email : [%s]', profile.email);
  Citizen.findByEmail(profile.email, function (err, citizen){
    if (err) return callback(err);
    if (!citizen) {
      return callback(new Error("No user for email " + profile.email));
    }
    log('Password reset requested for citizen [%s]', citizen.id);
    api.passwordResetToken.create(citizen, function (err, passwordResetToken) {
      if (err) return callback(err);
      log('PasswordResetToken created %j', passwordResetToken);
      // TODO send email with mandrill
      return callback(err, citizen); 
    });
    return callback(err, citizen);     
