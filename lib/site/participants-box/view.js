...
export default class ParticipantsBox extends View {
  ...
  fetch () {
    ...
    request
      ...
      .send({ids: this.participants.map((u) => u.id)})
      .end((err, res) => {
        if (err || !res.ok) {
  ...
  load (participants) {
    this.find('a.participant-profile').remove()
    
    const container = document.createDocumentFragment()
    participants.forEach((p, i) => {
      new Participant(p, i).appendTo(container)
    })

    this.find('.participants-container')[0].appendChild(container)
