...
class HomeMultiForum extends Component {
  ...
  handleLoadMoreSearch = () => {
    forumStore.search(this.state.query, this.state.page)
    ...
    .catch((err) => {
      this.setState({ isLoadingSearch: false, noMoreResults: true })
      log('Found error %s', err)
    })
  }
  ...
  search = (query) => {
    this.setState({ isLoadingSearch: true, query })
    forumStore.search(query)
    .then((res) => res.results.forums)
    .then((searchResults) => {
      this.setState({ isSearching: true, searchResults, isLoadingSearch: false, page: this.state.page + 1 })
    })
    .catch((err) => {
  ...
  render () {
    ...
    if (this.state.isSearching) {
      section = (
        <SearchResults noMoreResults={this.state.noMoreResults} forums={this.state.searchResults} handleLoadMoreSearch={this.handleLoadMoreSearch} />
      )
    } else {
      section = (
        <div>
