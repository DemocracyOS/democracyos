...
class HomeMultiForum extends Component {
  ...
  isSearching = (text) => {
    ...
    if (text.length > 2) {
      this.timer = setTimeout(this.search.bind(this, text), 250)
    }
    if (text.length === 0) {
  ...
  search = (query) => {
    ...
    this.forumStore.search(query)
    .then((searchResults) => this.setState({ searchResults, isLoadingSearch: false }))
    .catch((err) => {
    ...
    return (
      <div id='home-multiforum' className='center-container'>
        ...
        <div id='forum-list'>
          ...
          <SearchBar isLoadingSearch={this.state.isLoadingSearch} isSearching={this.isSearching} />
          {section}
        </div>
      </div>
    )
