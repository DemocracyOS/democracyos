...
export class Poll extends Component {
    ...
    topicStore.vote(this.props.topic.id, this.state.selected)
      .then(() => {
        this.setState((prevState) => ({
          changingVote: !prevState.changingVote
        }))
      })
      .catch((err) => { throw err })
  }
  ...
  render () {
    ...
    const { results, showResults, changingVote } = this.state
    ...
    return (
      <div className='topics-poll'>
        <div className='poll-options'>
          {!changingVote && results.map((result, i) => (
            <ResultBox
              key={i}
          ...
          {!changingVote && <ChangeVote handleClick={this.changeVote} /> }
          {
            changingVote && results.map((result, i) => (
              <Option
                key={i}
