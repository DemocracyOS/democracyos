...
export default class DecisionMaker extends Component {
  ...
  resultChartDidMount (resultChart) {
    ...
    if (votesTotal) {
      ...
      new Chart(resultChart.getContext('2d')).Pie(data, {animation: false})
    }
  }
  ...
  render () {
                ...
                .map((o) => {
                  ...
                  return (
                    <a
                      ...
                      onClick={this.vote}
                      key={o}>
                      <i className={option.button.icon}></i>
    ...
    return (
      <div className='proposal-options topic-article-content'>
        ...
        {closed ? resultBox : (!this.state.voted || this.state.changingVote) ? voteBox : null}
