...
if (!config('users whitelist')) return;
...
app.post('/whitelists/create', staff, function (req, res) {
  log('Request /whitelists/create %j', req.body);

  api.whitelist.create(req.body, function (err, whitelist) {
...
app.post('/whitelists/:id', staff, function (req, res) {
  ...
  api.whitelist.update(req.params.id, req.body, function (err, whitelist) {
    ...
    log('Serving whitelist %s', whitelist.id);
    res.json(whitelist.toJSON());
  });
});

app.del('/whitelists/:id', staff, function (req, res) {
  var id = req.params.id;
  log('Request /whitelists/%s', id);
