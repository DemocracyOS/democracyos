...
export default class Store {
  ...
  /**
   * Method to find a list of Models from the Database.
   *
   * @param {String} id
   * @return {Promise} fetch
   * @api public
  ...
  findAll (...args) {
    ...
    if (this._findAllCache.url === url) return Promise.resolve(this._findAllCache.items);
    if (this._fetches.get(url)) return this._fetches.get(url);
