...
export default class TopicForm extends FormView {
  ...
  constructor(topic, forum) {
    super();
    this.setLocals(topic, forum);
    super(template, this.locals);

    if (tagStore.items.length == 0) return;
  ...
  onsuccess(response) {
    ...
    if (response.req.url.match(/.*create.*/)) {
      created = true;
      page(this.forumAdminUrl + '/topics/' + response.body.id);
    }
    this.messages([t('admin-topics-form.message.onsuccess')]);
  ...
  ondeletetopicclick(ev) {
    ...
    function onconfirmdelete(ok) {
      ...
      topicStore
        .destroy(this.topic.id)
        .then(() => {
          topicStore
            .findAll()
            .then(() => {
              page(this.forumAdminUrl);
            })
            .catch(err => {
              log('Found error %o', err);
            })
