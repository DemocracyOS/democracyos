...
CommentsRepliesView.prototype.onsuccessedit = function(data) {
  ...
  replyTime.setAttribute('data-time', data.editedAt.toString());
  ...
  if (!edited) {
    ...
    replyTime.parentNode.insertBefore(small, replyTime);
  }

  var btn = o('.btn-edit', replyEl);
  classes(btn).remove('hide');
  classes(el.parentNode).remove('edit');
...
CommentsRepliesView.prototype.onconfirmremove = function(ev) {
  ...
  request
    ...
    .end(function(err, res) {
      ...
      var messageEl = o('.message', replyEl);
