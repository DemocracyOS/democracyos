...
ListView.prototype.switchOn = function () {
  ...
  bus.on('participants:updated', function(participants) {
    var participantsDiv = o('li.active a span.created-by div', this.el);
    if(!participantsDiv) return;

    var plural = 1 === participants.length ? '' : 's'
    participantsDiv.innerHTML = participants.length + ' ' + t('Participant') + plural
  }.bind(this));
