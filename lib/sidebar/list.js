...
var createdBy = require('./created-by');
...
ListView.prototype.vote = function (id) {
  ...
  if (itemLink) {
    ...
    var createdByEl = o('.created-by', itemLink);
    if (createdByEl) {
      filter.fetch();
      var  item = null;
      filter.items().forEach( function(it) {
        if (it.id === id) {
          item = it;
        }
      });
      if (item) {
        ...
        item.participants.forEach(function (participant) {
          if (participant == citizen.id) {
            alreadyVoted = true;
          }
        });
        if (!alreadyVoted) {
          item.participants = item.participants.concat(citizen.id);
        }
        createdByEl.appendChild(render.dom(createdBy, { item: item }));
      }
    }
  } 
  filter.vote(id);
