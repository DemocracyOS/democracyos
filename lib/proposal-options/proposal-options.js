...
function ProposalOptions (proposal, citizen) {
  ...
  this.events.bind('click #why', 'tipwhy');
...
/**
 * Tip why results are not shown
 *
 * @return {ProposalOptions} instance of `ProposalOptions`
 * @api public
 */

ProposalOptions.prototype.tipwhy = function(ev) {
  ev.preventDefault();
  var why = ev.target;
  if (why) {
    var message =   '<p>' + t("To not take part on the citizen's decision") + '</p>';
    message     +=  '<p>' + t("To respect parlament methodology") + '</p>';
    var tip = new Tip(message);
    tip.show(why, tip.hide(3500));
  }
  return this;
}
