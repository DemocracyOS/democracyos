...
function ProposalOptions (proposal, citizen) {
  ...
  this.events.bind('click', 'whyResults');
...
ProposalOptions.prototype.whyResults = function(ev) {
  ev.preventDefault();
  var why = o('.proposal-options .why-results', this.el);
  var reasons = o('.reasons');
  if (reasons) {
    if (ev.target !== why) {
      classes(reasons).add('hide');
    } else {
      classes(reasons).toggle('hide');
    }
  }
}

ProposalOptions.prototype.toggleReason = function(ev, reason) {
