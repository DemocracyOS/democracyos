...
function canCreate (req, res, next) {
  req.user.privileges = {}
  req.user.privileges.canCreate = config.restrictForumCreation
    ? req.user.staff
    : true
  next()
}
...
function canManage (req, res, next) {
  ...
  var opt = {}
  opt['privileges.canChangeTopics']
  api.forum.all(opt, function (err, forums) {
    if (err) return next(err)
    req.user.privileges.canManage = config.restrictForumCreation
      ? !!forums.length
      : true
    next()
  })
...
app.get('/user/me', restrict, canCreate, canManage, function (req, res) {
  log('Request user/me')
