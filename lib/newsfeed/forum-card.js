...
import template from './forum-card-template.jade';
...
export default class ForumCard extends view('appendable') {
  constructor(options = {}) {
    ...
    setTimeout(this.renderBackground.bind(this), 0);
  }

  renderBackground() {
    const opts = {
      seed: parseInt(this.forum.id.slice(0, 8), 16),
      palette: palette
    };

    var pattern = new Trianglify(opts);
    dom(this.el).css('background', `url(${pattern.png()}) no-repeat center center`);
  }
}
