...
page("/help/:page?", valid, parse, function(ctx, next) {
  ...
  if(config['faq enabled']) faq.appendTo(content);
  if(config['tos enabled']) tos.appendTo(content);
  if(config['pp enabled']) pp.appendTo(content);
  if(config['glossary enabled']) glossary.appendTo(content);
