...
function canVoteAndComment(forum, user) {
  return !config.multiForum || !forum.private || forum.isOwner(user) || forum.hasRole('admin', user) || forum.hasRole('collaborator', user);
}
...
app.get('/forum/:id/permissions', function (req, res) {
  ...
  api.forum.getPermissions(req.params.id, function (err, permissions) {
    if (err) return _handleError(err, req, res);
    var userExpose = expose(userKeys);
    res.status(200).json(permissions.map(p => {
      return {role: p.role, user: userExpose(p.user)};
    }));
  });
});
...
app.delete('/forum/:id/permissions',
  ...
  restrictPermissionsChange,
  function (req, res) {
    api.forum.revokePermission(req.params.id, req.body.user)
      .then(() => res.status(200).send())
      .catch(err => {
        log('Found error: ', err);
        res.status(500).send();
