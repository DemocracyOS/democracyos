...
app.post('/forum/:id/permission', restrict, restrictPermissions, function (req, res, next) {
  api.forum.grantPermission(req.params.id, req.body.user, req.body.role)
  .then(forum => res.status(200).send(forum))
  .catch(err => res.status(500).send(err));
});

app.del('/forum/:id/permission', restrict, restrictPermissions, function (req, res, next) {
...
function restrictPermissions (req, res, next) {
  function isAllowed(user, role, forum) {
    return ('admins' === role && forum.owner.id === user.id)
      || ('collaborators' === role
          && forum.permissions.admin.map(function (a) { return a.toString(); }).indexOf(user.id) >= 0);
  }
  ...
  api.forum.findById(req.params.id, function (err, forum) {
    ...
    if (!req.body.role) return res.status(400).send();
    if (isAllowed(req.user, req.body.role, forum)) {
      next();
    } else {
