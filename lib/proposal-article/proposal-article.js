...
function ProposalArticle (proposal) {
  ...
  citizen.ready(function () {
    ...
    var sideComments = self.sideComments = new SideComments('.proposal.commentable-container', userComment, [
    {
...
ProposalArticle.prototype.onClauseCommentPosted = function(comment) {
  var self = this;
  var clauseComment = {};
  clauseComment.clause = comment.sectionId;
  clauseComment.comment = comment.comment;
  request
  .post('/api/law/:lawId/clauses/:clauseId/comment'.replace(':lawId', this.proposal.id).replace(':clauseId', comment.sectionId))
  .send(clauseComment)
  .end(function(err, res) {
    ...
    comment.sectionId = comment.clauseId;
