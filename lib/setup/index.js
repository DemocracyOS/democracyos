...
function Setup(app) {
  ...
  var mode = config('ssl').mode;

  if (ssl) {
    switch (mode) {
      case 'normal':
        app.use(enforce.HTTPS());
        log('Using redirection to HTTPS');
        break;
      case 'heroku':
        app.use(enforce.HTTPS(true));
        log('Using redirection to HTTPS compatible with Heroku/Nodejitsu');
        break;
      case 'azure':
        app.use(enforce.HTTPS(false, true));
        log('Using redirection to HTTPS compatible with Azure');
        break;
      default:
        log('SSL is enabled but no redirection has been set up. It\'s ok if you are in a development environment');
        break;
    }
  }
