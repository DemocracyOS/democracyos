...
function Setup(app) {
  ...
  app.configure(function() {
     ...
     * JSON Web Tokens
     */
    app.set('jwtTokenSecret', 'democracyos.org');

    app.use(function(req, res, next) {
      req.jwtAuthenticated = function(cb) {
        log('looking for a JSON Web Token');
        // For flexibility, we'll accept the token in one of three ways
        var token = (this.body && this.body.access_token)
          || (this.query && this.query.access_token)
          || this.headers['x-access-token'];

        if (token) {
          try {
            log('token found, attempting to decode...');
            var decoded = jwt.decode(token, app.get('jwtTokenSecret'));
            if (decoded.exp <= Date.now()) {
              //res.end('Access token has expired', 400);
              log('access token has expired');
              cb(new Error('Access token has expired'));
            }
    ...
    /*app.use(function awtAuth(req, res, next) {
      log('looking for JSON Web Token for all requests');
      // For flexibility, we'll accept the token in one of three ways
      ...
      if (token) {
        try {
          ...
          req.user = User.findByEmail(decoded.iss);
          log('token sucessfully decoded for user %s', decoded.iss);
        } catch (err) {
          log('cannot decode token: %s', err)
          return next();
        }
      } else {
        log('token not provided, moving on...')
        return next();
      }
    });*/
