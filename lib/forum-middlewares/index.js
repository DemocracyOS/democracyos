...
var roles = require('../models/forum/roles');
...
module.exports.private = function (req, res, next) {
  ...
  if (forum.visibility !== 'private') return next();
  ...
  log('Forum is private');
  return res.status(403).json({ message: 'Forum is private.' });
};
...
module.exports.restrictPermissionsView = function (req, res, next) {
  ...
  if (!user || !forum) {
    if (!user) log('User not found.');
    if (!forum) log('User not found.');
    return res.status(400).send();
  }
...
module.exports.restrictPermissionsChange = function (req, res, next) {
  ...
  if (!user || !forum || !role || !~roles.indexOf(role)) {
    log('Invalid arguments for restrictPermissionsChange.');
    return res.status(400).send();
  }

  if (forum.isOwner(user) || forum.hasRole(user, 'admin')) {
    return next();
  } else {
    return res.status(403).send({error: 'You can\'t change this permissions'});
  }
